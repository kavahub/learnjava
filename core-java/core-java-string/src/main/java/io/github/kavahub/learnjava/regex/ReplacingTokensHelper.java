...
public class ReplacingTokensHelper {
    public static final Pattern TITLE_CASE_PATTERN = Pattern.compile("(?<=^|[^A-Za-z])([A-Z][a-z]*)(?=[^A-Za-z]|$)");

    /**
     * Iterate over the title case tokens in the input and replace them with
     * lowercase
     * 
    ...
    public static String replaceTitleCaseWithLowerCase(String original) {
        ...
        while (matcher.find()) {
            output.append(original, lastIndex, matcher.start()).append(convert(matcher.group(1)));
