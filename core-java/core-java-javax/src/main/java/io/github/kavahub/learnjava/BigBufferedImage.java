...
/**
 * {@link BigBufferedImage} can load and store much larger images than the memory limit. 
 * The trick is that its buffer is replaced with a file based implementation of DataBuffer. 
 * It stores the decoded raw image data in files on the hard drive.
 * 
 * <p>
 * The buffer files are accessed with a MappedByteBuffer, which provides 
 * a fast random access to the files, you can handle the files as simple byte arrays in the memory.
 * 
 * <p>
 * With {@link BigBufferedImage} you can store maximum 2,147,483,647 pixels (or 46,340 x 46,340 pixels). 
 * With 4 color channels (RGB) it means c.a. 8 GByte raw data. It has the same theoretical 
