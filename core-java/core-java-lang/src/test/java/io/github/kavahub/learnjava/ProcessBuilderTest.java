...
public class ProcessBuilderTest {
    ...
    public void givenProcessBuilder_whenInvokeStart_thenSuccess()
        ...
        assertThat("No errors should be detected", exitCode, is(0));
    }
    ...
    public void givenProcessBuilder_whenRedirectStandardOutput_thenSuccessWriting()
        ...
        processBuilder.redirectErrorStream(true);
        File log = directory.resolve("java-version.log").toFile();
        processBuilder.redirectOutput(log);

        Process process = processBuilder.start();

        assertThat("If redirected, should be -1 ", process.getInputStream().read(), is(-1));
        int exitCode = process.waitFor();
        assertThat("No errors should be detected", exitCode, is(0));
    ...
    public void givenProcessBuilder_whenRedirectStandardOutput_thenSuccessAppending()
        ...
        File log = directory.resolve("java-version-append.log").toFile();
        processBuilder.redirectErrorStream(true);
    ...
    public void givenProcessBuilder_whenStartingPipeline_thenSuccess() throws IOException, InterruptedException {
        assumeTrue(!isWindows(), () -> "只能在Linux环境中运行");

        List<ProcessBuilder> builders = Arrays.asList(
                new ProcessBuilder("find", "src", "-name", "*.java", "-type", "f"), new ProcessBuilder("wc", "-l"));
